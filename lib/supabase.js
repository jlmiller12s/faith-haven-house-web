...
//
// IMPORTANT: in the browser we MUST use the cookie-aware @supabase/ssr client
// so crmService queries carry the same authenticated session as the RAP login
// flow (which authenticates via @supabase/ssr cookies). A plain createClient()
// here keeps its session in separate storage and silently queries as `anon`,
// which RLS/grants correctly reject ("permission denied for table ...").
