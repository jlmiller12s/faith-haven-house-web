...
export async function getAdmissionsQueue() {
  ...
  return (data || []).map(c => ({
    ...
    assignedCoordinatorName: c.staff_profiles ? `${c.staff_profiles.first_name} ${c.staff_profiles.last_name}` : "Unassigned",
...
export async function getCaseDetails(caseId) {
  ...
  const { data: caseObj } = await supabase.from("admissions_cases").select("*").eq("id", caseId).single();
  if (!caseObj) return null;

  const { data: applicant } = await supabase.from("applicants").select("*").eq("id", caseObj.applicant_id).single();
  const { data: prescreen } = await supabase.from("prescreen_submissions").select("*").eq("applicant_id", caseObj.applicant_id).single();
  const { data: documents } = await supabase.from("admissions_documents").select("*").eq("admissions_case_id", caseId);
  const { data: interview } = await supabase.from("admissions_interviews").select("*").eq("admissions_case_id", caseId).single();
  const { data: clinical } = await supabase.from("behavioral_health_reviews").select("*").eq("admissions_case_id", caseId).single();
  const { data: decision } = await supabase.from("committee_decisions").select("*").eq("admissions_case_id", caseId).single();
  const { data: welcome } = await supabase.from("welcome_day_records").select("*").eq("admissions_case_id", caseId).single();
  const { data: tasks } = await supabase.from("tasks").select("*").eq("admissions_case_id", caseId);
  const { data: notes } = await supabase.from("notes").select("*").eq("admissions_case_id", caseId);
  const { data: timeline } = await supabase.from("activity_events").select("*").eq("admissions_case_id", caseId);
  const { data: auditLogs } = await supabase.from("audit_logs").select("*").eq("admissions_case_id", caseId);
