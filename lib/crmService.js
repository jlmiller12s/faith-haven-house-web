...
export async function getCaseDetails(caseId) {
  ...
  if (caseError) throw new Error(`Unable to load admission case: ${caseError.message}`);
  ...
  const { data: prescreen } = await supabase.from("prescreen_submissions").select("*").eq("admissions_case_id", caseId).maybeSingle();
  const { data: documents } = await supabase.from("admissions_documents").select("*").eq("admissions_case_id", caseId);
  const { data: interview } = await supabase.from("admissions_interviews").select("*").eq("admissions_case_id", caseId).maybeSingle();
  const { data: clinical } = await supabase.from("behavioral_health_reviews").select("*").eq("admissions_case_id", caseId).maybeSingle();
  const { data: decision } = await supabase.from("committee_decisions").select("*").eq("admissions_case_id", caseId).maybeSingle();
  const { data: welcome } = await supabase.from("welcome_day_records").select("*").eq("admissions_case_id", caseId).maybeSingle();
