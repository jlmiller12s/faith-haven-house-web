...
import { buildDocumentStatusPayload, persistDocumentStatus } from "./documentStatus.mjs";
...
export async function updateDocumentStatus(params, actorId) {
  ...
  if (isMockMode) {
    ...
    Object.assign(doc, buildDocumentStatusPayload({
      status,
      actorId,
    ...
    const persistenceResult = await persistDocumentStatus({
      caseId,
      documentTypeId,
      status,
      actorId,
      notes,
      saveDocument: async (payload) => supabase
        .from("admissions_documents")
        .upsert(payload, { onConflict: "admissions_case_id,document_type_id" })
        .select("id")
        .single()
    });
    ...
    await logAuditEvent({
      ...
      entityType: "admissions_document",
      entityId: persistenceResult.documentId,
      caseId,
      metadata: { status }
    });
    return { success: true };
...
export async function deleteStaff(profileIds, actorId) {
  if (isMockMode) {
    const db = getMockDb();
    db.staff = db.staff.filter(s => !profileIds.includes(s.id));
    return { success: true };
  }
  ...
  const res = await fetch("/api/staff/delete", {
    ...
    body: JSON.stringify({ profileIds, actorId }),
  });
